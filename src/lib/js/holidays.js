...
export const HOLIDAY_DETAILS = {
	...
	yahrtzeit_of_rabbi_yosef_yitzchak_schneersohn: {
		name: "Yahrtzeit of Rabbi Yosef Yitzchak Schneersohn and Beginning of Rebbe's Leadership",
		start: { mm: Shevat, dd: 10 },
		end: { mm: Shevat, dd: 10 },
		type: 'chabad',
	},
		...
		for (const [hID, holiday] of Object.entries(HOLIDAY_DETAILS)) {
			...
			if (types[holidayType] ?? true) {
				...
				if (holidayType == 'shabbosMevorchim' || hID == 'leil_selichos' || holidayType == 'specialShabbos') {
					...
					if (
						!(
							(hID == 'shabbos_shekalim' ||
								hID == 'shabbos_parah' ||
								hID == 'shabbos_hachodesh' ||
								hID == 'shabbos_shira' ||
								hID == 'shabbos_shuva' ||
								hID == 'shabbos_chazon' ||
								hID == 'shabbos_nachamu') &&
				...
				let wrongRoshChodeshAdar =
					((7 * year + 1) % 19 < 7 && (hID == 'rosh_chodesh_adar' || hID == 'shabbos_mevorchim_adar')) ||
					((7 * year + 1) % 19 >= 7 && (hID == 'rosh_chodesh_adar_i' || hID == 'rosh_chodesh_adar_ii' || hID == 'shabbos_mevorchim_adar_i' || hID == 'shabbos_mevorchim_adar_ii'));
